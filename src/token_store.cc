...
int TokenStore::Read() {
    if (is_remote_backend_) return ReadFromUrl();
    return ReadFromFile();
}

int TokenStore::Write() {
    if (is_remote_backend_) return WriteToUrl();
    return WriteToFile();
}

int TokenStore::ReadFromFile() {
  try {
